...
namespace LKMT.GUI
{
    public partial class fSanPham : UserControl
    {
        ...
        {
            InitializeComponent();
            SanPhamBLL.Instance.showSanPham(dgvSanPham);
            ...
            NhomSanPhamBLL.Instance.showListNhomSP(cboNhomLK);
        }
        ...
        private void dgvSanPham_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (selectedRowCount > 0)
            {
                if (e.RowIndex != -1)
                {
                    ...
                    SanPhamBLL.Instance.showFromGridviewToCBO(row.Cells[4].Value.ToString(), int.Parse(row.Cells[3].Value.ToString()), cboNhomLK, cboLoaiLK,cboThuongHieu);
                    nmrKhuyenMai.Value = int.Parse(row.Cells[5].Value.ToString());
                    nmrBaoHanh.Value = int.Parse(row.Cells[6].Value.ToString());
                    SanPhamBLL.Instance.showImageToPictureBox(row.Cells[7].Value.ToString(), pictureLinhKien);
        ...
        private void cboNhomLK_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            SanPhamBLL.Instance.showComboboxChanged(dgvSanPham, cboNhomLK, cboLoaiLK, cboThuongHieu);
        }
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (SanPhamBLL.Instance.themSanPham(txtTenLinhKien.Text, cboLoaiLK, decimal.Parse(txtGia.Text), cboThuongHieu, (int)nmrBaoHanh.Value, (int)nmrKhuyenMai.Value, lbPath.Text, richMoTa.Text))
                {
                    MessageBox.Show("Thêm linh kiện thành công!!", "Thông Báo", MessageBoxButtons.OK);
                    SanPhamBLL.Instance.showSanPham(dgvSanPham);
                    if(SanPhamBLL.Instance.isExistImage(lbPath.Text) == false)
                    {
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            if (selectedRowCount>0)
            {
                if (SanPhamBLL.Instance.suaSanPham(txtMaLinhKien.Text, txtTenLinhKien.Text, cboLoaiLK, decimal.Parse(txtGia.Text), cboThuongHieu, (int)nmrBaoHanh.Value, (int)nmrKhuyenMai.Value, lbPath.Text, richMoTa.Text, DateTime.Parse(txtNgayTao.Text)))
                {
        ...
        {
            if (SanPhamBLL.Instance.xoaNhomSP(txtMaLinhKien.Text))
            {
